...
simulatedbatchWeibull_asymptoticbias<-foreach(batchnumber = (1:length(allkurtWeibull)), .combine = 'rbind') %dopar% {
  ...
  sortedx<-c()
  allrawmoBias<-c(
    firstbias=abs(c(Huberx,SMWM9,midhinge1,SWA81,SWAHlmean1,rqm1)-targetm)/(sqrt(targetvar))
  )
  
  all1<-(c(kurtx,Huberx,SMWM9,midhinge1,SWA81,SWAHlmean1,rqm1,targetall,momentsx,allrawmoBias))
}
